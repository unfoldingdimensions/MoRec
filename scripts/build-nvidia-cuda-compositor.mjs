...
const bundledExePath = path.join(bundledDir, "morec-nvidia-cuda-compositor.exe");
const helperId = "morec-nvidia-cuda-compositor";
const generatorArch = process.arch === "arm64" ? "ARM64" : "x64";
const videoCodecSdkRoot =
	process.env.MOREC_NVIDIA_VIDEO_CODEC_SDK_ROOT?.trim() ||
...
function fallbackToBundledHelperOrExit(reason) {
	if (existsSync(bundledExePath)) {
		const verification = verifyNativeHelperManifest({
			...
			binaryName: "morec-nvidia-cuda-compositor.exe",
		});
		if (!verification.ok) {
			console.warn(
...
if (!existsSync(path.join(videoCodecSdkRoot, "Samples", "NvCodec"))) {
	fallbackToBundledHelperOrExit(
		`NVIDIA Video Codec SDK samples not found at ${videoCodecSdkRoot}. Set MOREC_NVIDIA_VIDEO_CODEC_SDK_ROOT to build from source.`,
	);
}
...
function patchNvDecoderForMoRecCallbacks() {
	...
	if (!header.includes("MoRecMappedFrameHandler")) {
...
using MoRecMappedFrameHandler = void (*)(CUdeviceptr, unsigned int, int, int, int, int64_t, void*);
using MoRecDisplayFramePolicy = bool (*)(int, void*);
`,
    ...
    void SetMappedFrameHandler(MoRecMappedFrameHandler handler, void* userData) { m_morecMappedFrameHandler = handler; m_morecMappedFrameUserData = userData; }
    void SetDisplayFramePolicy(MoRecDisplayFramePolicy policy, void* userData) { m_morecDisplayFramePolicy = policy; m_morecDisplayFramePolicyUserData = userData; }
...
`,
    ...
    MoRecMappedFrameHandler m_morecMappedFrameHandler = nullptr;
    void* m_morecMappedFrameUserData = nullptr;
    MoRecDisplayFramePolicy m_morecDisplayFramePolicy = nullptr;
    void* m_morecDisplayFramePolicyUserData = nullptr;
`,
	...
	if (!source.includes("MoRec mapped frame callback")) {
    ...
    if (m_morecDisplayFramePolicy &&
        !m_morecDisplayFramePolicy(displayFrameIndex, m_morecDisplayFramePolicyUserData))
    {
        NVDEC_API_CALL(cuvidUnmapVideoFrame(m_hDecoder, dpSrcFrame));
        return 1;
    }

    // MoRec mapped frame callback keeps the CUDA helper from making an
    // extra device-to-device copy when the caller can consume mapped NV12.
    if (m_morecMappedFrameHandler)
    {
        m_morecMappedFrameHandler(
            dpSrcFrame,
            nSrcPitch,
            m_nWidth,
            m_nHeight,
...
try {
	patchNvDecoderForMoRecCallbacks();
} catch (error) {
	fallbackToBundledHelperOrExit(
...
try {
	configureWithWindowsCmakeGenerator({
		...
		configure: (generator, toolset) =>
			execSync(
				`${cmake} .. -G "${generator}" -A ${generatorArch}${toolset ? ` -T ${toolset}` : ""} -DMOREC_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
				{
					cwd: buildDir,
					stdio: "inherit",
					timeout: 120000,
...
const exePath = path.join(buildDir, "Release", "morec-nvidia-cuda-compositor.exe");
