...
const productName = packageJson.productName ?? packageJson.name ?? "Mo Rec";
const packageName = packageJson.name ?? "morec";
...
function getExpectedNativeHelperFiles(archTag) {
	if (archTag.startsWith("win32-")) {
		...
		if (archTag === "win32-x64") {
			helpers.push({
				name: "morec-nvidia-cuda-compositor.exe",
	...
	if (archTag.startsWith("darwin-")) {
		return [
			{
				name: "morec-screencapturekit-helper",
				label: "ScreenCaptureKit helper",
				executable: true,
			},
			{ name: "morec-window-list", label: "Window list helper", executable: true },
			{ name: "morec-system-cursors", label: "System cursor helper", executable: true },
			{
				name: "morec-native-cursor-monitor",
