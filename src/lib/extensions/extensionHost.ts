...
const EXTENSION_SETTINGS_STORAGE_KEY = "morec.extension-settings.v1";
...
export class ExtensionHost {
	...
	async activateExtension(info: ExtensionInfo, moduleUrl: string): Promise<void> {
		...
		let mod: MoRecExtensionModule | null = null;
		try {
			...
			try {
				const loaded: MoRecExtensionModule = await import(/* @vite-ignore */ moduleUrl);
				mod = loaded;
