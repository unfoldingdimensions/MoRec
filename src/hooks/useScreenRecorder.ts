...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
		...
		if (!screenPermission.success || screenPermission.status !== "granted") {
			...
			alert(
				options.startup
					? "Mo Rec needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen Mo Rec."
					: "Screen Recording permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Mo Rec before recording.",
			);
			return false;
		}
		...
		alert(
			options.startup
				? "Mo Rec also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen Mo Rec."
				: "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Mo Rec before recording.",
		);
		...
		if (nativeScreenRecording.current) {
			...
			void (async () => {
				...
				try {
					...
					console.log(
						"[useScreenRecorder] Native processing: webcamPath is",
						webcamPath,
					);
					...
					console.error("Error finalizing native recording session:", error);
					await notifyRecordingFinalizationFailure(
						`Failed to finalize the recording session. ${error instanceof Error ? error.message : String(error)}`,
					);
				} finally {
					if (typeof window.electronAPI?.hudOverlayClose === "function") {
						console.log(
							"[useScreenRecorder] All tasks finished, closing HUD",
						);
						window.electronAPI.hudOverlayClose();
					}
				}
	...
	useEffect(() => {
		...
		void (async () => {
			...
			if (result.success) {
				...
				if (typeof result.webcamEnabled === "boolean") {
				...
				try {
					...
					if (videoResult.path) {
						...
						try {
							...
							if (typeof window.electronAPI?.hudOverlayClose === "function") {
								console.log(
									"[useScreenRecorder:browser] All tasks finished, closing HUD",
								);
								window.electronAPI.hudOverlayClose();
							}
						}
