...
export default function VideoEditor() {
			...
			try {
				...
				if (settings.format === "gif" && settings.gifConfig) {
					...
					if (result.success && (result.blob || result.tempFilePath)) {
						...
						// User cancelled — handleCancelExport already reset the UI; do not
						// surface the cancelled export as an error.
					} else {
						if (smokeExportConfig.enabled) {
							await writeSmokeExportReport(smokeExportConfig.outputPath, {
								success: false,
	...
	useEffect(() => {
		...
		// When smoke-export opens a .morec project, the cursor telemetry
					...
					<DialogHeader>
						...
						<DialogDescription className="text-muted-foreground">
							{t(
								...
								"Name this project. It will be saved in your Mo Rec Projects folder.",
							)}
						</DialogDescription>
					</DialogHeader>
					<div className="py-4">
				...
				<DialogHeader>
					...
					<DialogDescription className="text-muted-foreground">
						{t(
							...
							"Your device does not support native capture. This could be for a variety of reasons we haven’t figured out yet. This doesn’t break Mo Rec, but it does make cursor smoothing impossible.",
						)}
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
	...
	return (
		<div className="flex flex-col h-screen bg-editor-bg text-foreground overflow-hidden selection:bg-[#2563EB]/30">
			...
			<PanelGroup
				direction="vertical"
				className="relative flex min-h-0 flex-1 flex-col gap-2 p-4"
			>
				<Panel defaultSize={82} minSize={40} className="flex min-h-0 flex-1 flex-col">
					<PanelGroup
						direction="horizontal"
						className="flex min-h-0 flex-1 gap-2 relative z-10"
					>
						{/* Settings sidebar */}
						<Panel
							defaultSize={26}
									...
									<div className="flex items-center gap-1.5">
										...
										<div className="relative flex h-7 w-24 select-none items-center overflow-hidden rounded-full border border-foreground/[0.06] bg-editor-bg/80 shadow-[inset_0_1px_0_hsl(var(--foreground)/0.06)]">
											...
											<div
												...
												style={{
													left: `calc(${previewVolume * 100}% - 8px)`,
												}}
											/>
