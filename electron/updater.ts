...
const AUTO_UPDATES_DISABLED = process.env.MOREC_DISABLE_AUTO_UPDATES === "1";
const UPDATE_FEED_URL_OVERRIDE = process.env.MOREC_UPDATE_FEED_URL?.trim() ?? "";
const UPDATER_LOG_PATH =
	process.env.MOREC_UPDATER_LOG_PATH?.trim() || path.join(USER_DATA_PATH, "updater.log");
	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. Mo Rec will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before Mo Rec restarts.`
					: "Downloading the update now. Mo Rec will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
...
export async function downloadAvailableUpdate(
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading Mo Rec ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `Mo Rec ${version} is available.`,
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `Mo Rec ${version} is ready to install.`
			: `Mo Rec ${version} has been downloaded.`,
		detail: isPreview
		...
		if (options?.manual) {
			await showMessageBox(getMainWindow, {
				...
				detail: AUTO_UPDATES_DISABLED
					? "This build disabled auto-updates through MOREC_DISABLE_AUTO_UPDATES=1."
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `Mo Rec ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `Mo Rec ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `Mo Rec ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
