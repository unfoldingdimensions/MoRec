	...
	ipcMain.handle(
		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
				...
				try {
					...
					if (options?.capturesSystemAudio) {
						...
						tempSystemAudioPath = path.join(
							app.getPath("temp"),
							`morec-native-${timestamp}.system.wav`,
						);
					...
					if (options?.capturesMicrophone && !browserMicFallbackRequested) {
						...
						tempMicPath = path.join(
							app.getPath("temp"),
							`morec-native-${timestamp}.mic.wav`,
						);
			...
			try {
				...
				if (
					!ALLOW_MOREC_WINDOW_CAPTURE &&
					source?.id?.startsWith("window:") &&
					appName &&
					(appName === ownAppName || appName === "morec" || appName === "mo rec")
				) {
					return {
						success: false,
						message: "Cannot record Mo Rec windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message: "Mo Rec needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Mo Rec is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Mo Rec needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Mo Rec is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
