...
export function registerProjectHandlers() {
		...
		async (
			...
			try {
				...
				const result = await dialog.showSaveDialog({
					title: "Save Mo Rec Project",
					defaultPath: path.join(projectsDir, defaultName),
					filters: [
						{ name: "Mo Rec Project", extensions: [PROJECT_FILE_EXTENSION] },
	...
	ipcMain.handle("delete-recording-file", async (_, filePath: string) => {
		try {
			...
			const baseWithoutExt = path.join(dir, baseName);
			...
			await Promise.all([
				fs.unlink(telemetryPath).catch(() => undefined),
				...sidecarSuffixes.flatMap((suffix) => [
					fs.unlink(`${baseWithoutExt}${suffix}`).catch(() => undefined),
					fs.unlink(`${resolvedPath}${suffix}`).catch(() => undefined),
				]),
			]);
