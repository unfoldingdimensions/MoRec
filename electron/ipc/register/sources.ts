		...
		if (process.platform !== "darwin" || !includeWindows) {
			const windowSources = electronSources
				...
				.filter((source) => {
					...
					if (
						(ALLOW_MOREC_WINDOW_CAPTURE && normalizedName.includes("morec")) ||
						normalizedName.includes("mo rec")
					) {
						return true;
					}
		...
		try {
			...
			const mergedWindowSources = nativeWindowSources
				.filter((source) => {
					...
					if (
						ALLOW_MOREC_WINDOW_CAPTURE &&
						(normalizedAppName === "morec" ||
							normalizedAppName === "mo rec" ||
							normalizedWindowName?.includes("morec") ||
							normalizedWindowName?.includes("mo rec"))
					) {
						return true;
					}
