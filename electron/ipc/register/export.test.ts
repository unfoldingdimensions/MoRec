...
async function makeTempDir() {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "morec-export-move-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("moveExportedTempFile", () => {
	it("moves an app-managed export temp file to the selected destination", async () => {
		...
		await fs.writeFile(tempPath, "morec-export");

		await moveExportedTempFile(tempPath, destinationPath);

		await expect(fs.readFile(destinationPath, "utf8")).resolves.toBe("morec-export");
