...
const NVIDIA_CUDA_EXPORT_ENV = "MOREC_EXPERIMENTAL_NVIDIA_CUDA_EXPORT";
const NVIDIA_CUDA_ALLOW_AUDIO_EXPORT_ENV = "MOREC_NVIDIA_CUDA_ALLOW_AUDIO_EXPORT";
const NVIDIA_CUDA_FORCE_VIDEO_ONLY_ENV = "MOREC_NVIDIA_CUDA_FORCE_VIDEO_ONLY";
const NVIDIA_CUDA_AUTO_STALL_TIMEOUT_ENV = "MOREC_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS";
const DEFAULT_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS = 120_000;
const NATIVE_GPU_STALL_TIMEOUT_ENV = "MOREC_NATIVE_GPU_STALL_TIMEOUT_MS";
...
function shouldPersistNativeExportDiagnostics() {
	const rawValue = process.env.MOREC_NATIVE_EXPORT_DIAGNOSTICS?.trim().toLowerCase();
...
function shouldPersistNvidiaCudaExportDiagnostics() {
	return (
		...
		process.env.MOREC_NVIDIA_CUDA_EXPORT_DIAGNOSTICS === "1"
	);
}
...
async function resolveExperimentalWindowsGpuExporterPath() {
	...
	const executableNames = ["morec-gpu-export.exe", "gpu-export-probe.exe"];
	const candidates: string[] = [];
	const configuredPath = process.env.MOREC_WINDOWS_GPU_EXPORT_EXE;
	if (configuredPath) {
...
export async function resolveExperimentalNvidiaCudaExportScriptPath() {
	...
	const candidates: string[] = [];
	const configuredPath = process.env.MOREC_NVIDIA_CUDA_EXPORT_SCRIPT;
	if (configuredPath) {
		candidates.push(configuredPath);
	}
	const resourcesPath = (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath;
...
function resolveExperimentalNvidiaCudaNodeCommand() {
	const configuredNodePath = process.env.MOREC_NVIDIA_CUDA_NODE_EXE;
...
export function buildExperimentalNvidiaCudaStaticLayoutArgs(
	...
	const args = [
		...
		"--stream-sync",
		"--prewarm-ms",
		process.env.MOREC_NVIDIA_CUDA_PREWARM_MS || "500",
		"--content-x",
		String(Math.round(options.offsetX)),
		"--content-y",
		String(Math.round(options.offsetY)),
	...
	if (process.env.MOREC_NVIDIA_CUDA_SAMPLE_GPU === "1") {
...
async function runExperimentalNvidiaCudaStaticLayoutExport(
	...
	const env = {
		...process.env,
		...nodeCommand.env,
		MOREC_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY:
			process.env.MOREC_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY ?? "1",
		MOREC_FFMPEG_EXE: ffmpegPath,
		MOREC_FFPROBE_EXE: process.env.MOREC_FFPROBE_EXE ?? getFfprobeBinaryPath(),
...
export async function probeNativeVideoEncoder(
	...
	const outputPath = path.join(
		app.getPath("temp"),
		`morec-export-probe-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
...
export async function muxExportedVideoAudioBuffer(
	...
	const tempVideoPath = path.join(
		app.getPath("temp"),
		`morec-export-video-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
	const metrics: NativeVideoAudioMuxMetrics = {};
	let succeeded = false;
