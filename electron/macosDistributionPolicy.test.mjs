...
Identifier=com.morec.app
CodeDirectory v=20500 size=123 flags=0x10000(runtime) hashes=3+7 location=embedded
Authority=Developer ID Application: MoRec Developer (A1B2C3D4E5)
...
describe("macOS distribution architecture policy", () => {
	...
	it("uses path-specific helper architecture before the build architecture", () => {
		...
		expect(expectedMachOArchitecture("MoRec.app/Contents/MacOS/MoRec", "x64")).toBe("x86_64");
	});

	it("reports a binary that lacks the required architecture", () => {
		expect(collectArchitectureErrors("MoRec.app/Contents/MacOS/MoRec", "arm64", "x64")).toEqual(
			["MoRec.app/Contents/MacOS/MoRec does not contain x86_64 (found: arm64)"],
		);
	});
});
