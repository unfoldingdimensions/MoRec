...
const IS_SMOKE_EXPORT = process.env.MOREC_SMOKE_EXPORT === "1";
...
function getPlatformAppIconFilename(size: 32 | 128 | 512) {
	const baseName = process.platform === "darwin" ? "morecmac" : "morec";
...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		...
		case "downloading":
			return `Downloading Mo Rec ${payload.version}`;
		case "ready":
			return `Mo Rec ${payload.version} is ready`;
		case "error":
			return `Mo Rec ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Mo Rec.";
		case "downloading":
			return "Mo Rec is downloading the update and will restart when it is ready.";
		case "ready":
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("com.morec.app");
	}
	...
	// Mo Rec does not use WebHID, Web Serial, or WebUSB. Do not grant devices by default.
	...
	if (IS_SMOKE_EXPORT || process.env.MOREC_DEV_OPEN_RECORDING_INPUT) {
		await logSmokeExportGpuDiagnostics();
		if (IS_SMOKE_EXPORT) {
			const smokeSource =
				process.env.MOREC_SMOKE_EXPORT_PROJECT ??
				process.env.MOREC_SMOKE_EXPORT_INPUT ??
				"<missing input>";
			console.log(`[smoke-export] Starting editor smoke export for ${smokeSource}`);
		} else {
			console.log(
				`[dev-open-recording] Starting editor for ${process.env.MOREC_DEV_OPEN_RECORDING_INPUT}`,
			);
		}
		createEditorWindowWrapper();
		return;
