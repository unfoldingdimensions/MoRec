 ...
 * Mo Rec marketplace API. Also provides admin review endpoints.
 */
...
const MARKETPLACE_API_BASE = "https://marketplace.morec.app/extensions/api/v1";
...
function getMarketplaceUrl(): string {
	...
	if (process.env.MOREC_MARKETPLACE_URL) return process.env.MOREC_MARKETPLACE_URL;
...
async function marketplaceFetch<T>(
	...
	try {
		const headers: Record<string, string> = {
			...
			"X-MoRec-Platform": process.platform,
		};

		// Attach admin key for privileged endpoints
		if (options.admin) {
			const key = getAdminKey();
			if (!key) throw new Error("Admin key not configured (set MOREC_ADMIN_KEY env var)");
...
export async function downloadAndInstallExtension(
	...
	try {
		...
		if (dirs.length === 1 && !existsSync(path.join(extractDir, "morec-extension.json"))) {
			manifestDir = path.join(extractDir, dirs[0].name);
		}

		// Verify manifest exists
		if (!existsSync(path.join(manifestDir, "morec-extension.json"))) {
			throw new Error(
				"Downloaded extension does not contain a morec-extension.json manifest",
			);
		}
		...
		fetch(`${getMarketplaceUrl()}/extensions/${encodeURIComponent(extensionId)}/download`, {
			method: "POST",
			headers: { "X-MoRec-Version": app.getVersion() },
		}).catch(() => undefined);
