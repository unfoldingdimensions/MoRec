...
const WINDOW_ICON_FILENAME = process.platform === "darwin" ? "morecmac-512.png" : "morec-512.png";
...
function getEditorWindowQuery(): Record<string, string> {
	...
	if (process.env.MOREC_DEV_OPEN_RECORDING_INPUT) {
		query.devOpenInput = process.env.MOREC_DEV_OPEN_RECORDING_INPUT;
	}
	if (process.env.MOREC_DEV_OPEN_RECORDING_WEBCAM) {
		query.devOpenWebcam = process.env.MOREC_DEV_OPEN_RECORDING_WEBCAM;
	}

	if (process.env.MOREC_SMOKE_EXPORT === "1") {
		query.smokeExport = "1";
		if (process.env.MOREC_SMOKE_EXPORT_INPUT) {
			query.smokeInput = process.env.MOREC_SMOKE_EXPORT_INPUT;
		}
		...
		if (process.env.MOREC_SMOKE_EXPORT_ENCODING_MODE) {
			query.smokeEncodingMode = process.env.MOREC_SMOKE_EXPORT_ENCODING_MODE;
		}
		if (process.env.MOREC_SMOKE_EXPORT_SHADOW_INTENSITY) {
			query.smokeShadowIntensity = process.env.MOREC_SMOKE_EXPORT_SHADOW_INTENSITY;
		}
		if (process.env.MOREC_SMOKE_EXPORT_WEBCAM_INPUT) {
			query.smokeWebcamInput = process.env.MOREC_SMOKE_EXPORT_WEBCAM_INPUT;
		}
		if (process.env.MOREC_SMOKE_EXPORT_WEBCAM_SHADOW) {
			query.smokeWebcamShadow = process.env.MOREC_SMOKE_EXPORT_WEBCAM_SHADOW;
		}
		if (process.env.MOREC_SMOKE_EXPORT_WEBCAM_SIZE) {
			query.smokeWebcamSize = process.env.MOREC_SMOKE_EXPORT_WEBCAM_SIZE;
		}
		if (process.env.MOREC_SMOKE_EXPORT_PIPELINE) {
			query.smokePipelineModel = process.env.MOREC_SMOKE_EXPORT_PIPELINE;
		}
		if (process.env.MOREC_SMOKE_EXPORT_BACKEND) {
			query.smokeBackendPreference = process.env.MOREC_SMOKE_EXPORT_BACKEND;
		}
		if (process.env.MOREC_SMOKE_EXPORT_RENDER_BACKEND) {
			query.smokeRenderBackend = process.env.MOREC_SMOKE_EXPORT_RENDER_BACKEND;
		}
		if (process.env.MOREC_SMOKE_EXPORT_MAX_ENCODE_QUEUE) {
			query.smokeMaxEncodeQueue = process.env.MOREC_SMOKE_EXPORT_MAX_ENCODE_QUEUE;
		}
		if (process.env.MOREC_SMOKE_EXPORT_MAX_DECODE_QUEUE) {
			query.smokeMaxDecodeQueue = process.env.MOREC_SMOKE_EXPORT_MAX_DECODE_QUEUE;
		}
		if (process.env.MOREC_SMOKE_EXPORT_MAX_PENDING_FRAMES) {
			query.smokeMaxPendingFrames = process.env.MOREC_SMOKE_EXPORT_MAX_PENDING_FRAMES;
		}
		if (process.env.MOREC_SMOKE_EXPORT_PROJECT) {
			query.smokeProject = process.env.MOREC_SMOKE_EXPORT_PROJECT;
		}
		if (process.env.MOREC_SMOKE_EXPORT_QUALITY) {
			query.smokeQuality = process.env.MOREC_SMOKE_EXPORT_QUALITY;
		}
		if (process.env.MOREC_SMOKE_EXPORT_FPS) {
			query.smokeFps = process.env.MOREC_SMOKE_EXPORT_FPS;
		}
	}
